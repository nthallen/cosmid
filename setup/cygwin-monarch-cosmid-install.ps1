...
# > PowerShell -ExecutionPolicy Bypass ./cygwin-monarch-cosmid-install.ps1
#
# ---------
# Uninstall:
# ---------
...
# > PowerShell -ExecutionPolicy Bypass ./cygwin-monarch-cosmid-install.ps1 -uninstall
...
cat 1>&2 <<'EOF'
# Usage:
#   ./monarch-cosmid-install.sh [ -E moudi:nthallen/keutsch-moudi.git ] [-n] [-h] [-S]
#
# -E <basename>:<URL>
...
exp_option='cosmid:nthallen/cosmid.git'
exp_base=''
exp_url=''
testmode=no
installagent=no
...
if [ $machine = Cygwin ]; then
  ...
  if [ -e "$cyglnk" ]; then
    ...
    [ "$cygperm" = "775" ] || {
      chmod 0775 "$cyglnk"
      echo "monarch-cosmid-install.sh: Permission fix applied for Cygwin Start Menu Link"
    }
  fi
  # Apply vi annoyance fix if it isn't already present
  [ -f ~/.exrc ] || {
    touch ~/.exrc
    echo "monarch-cosmid-install.sh: Installed vi startup error mitigation"
  }
  # Verify that the monarch group flight exists and user is a member
  id | grep -q '(flight)' || {
    if id | grep -q "(`hostname`+flight)"; then
      echo "monarch-cosmid-install.sh: The flight group 'flight' has been created,"
      ...
      echo "monarch-cosmid-install.sh: user '$myuser' does not appear to be a member"
      echo "of group 'flight'. If you have not run cygwin-monarch-cosmid-install.ps1,"
      echo "do so now. Otherwise try restarting the system and then rerun"
      echo "cygwin-monarch-cosmid-install.ps1."
    fi
    echo
  ...
  if grep -q "^flight:" /etc/passwd; then
    ...
    echo "monarch-cosmid-install.sh: Need to create user flight and group flight"
    $sudo addgroup flight
    echo "monarch-cosmid-install.sh: flight group created"
    $sudo adduser --disabled-password --gecos "flight user" --no-create-home --ingroup flight flight
    echo "monarch-cosmid-install.sh: flight user created"
  fi
...
umask 02
echo "monarch-cosmid-install.sh: checking permissions on /usr/local/src"
...
if [ $machine = Linux ]; then
  ...
  if [ $distro = Ubuntu ]; then
    echo "monarch-cosmid-install.sh: Checking prerequisites"
    sudo apt install cmake doxygen graphviz gdb gcc g++ git bison flex libncurses-dev openssh-server screen
  else
    echo "monarch-cosmid-install.sh: Not specifically configured for release '$release'."
...
'@
$setup_script | Out-File -FilePath "./monarch-cosmid-install.sh" -NoNewLine -Encoding ASCII

Write-Output "`nStarting standard install script: /usr/local/src/monarch-cosmid-install.sh`n"

# -Wait won't work here, because mintty.exe doesn't really exit until ssh-agent does
start-process C:\cygwin64\bin\mintty.exe -argumentlist "-h always /bin/bash --login /usr/local/src/monarch-cosmid-install.sh -S $exp_option"
